...
\begin{figure}
	\centering
	\subcaptionbox{Conventional gait rehabilitation. \cite{gaitrehabgantry}\label{fig:gantry}}[0.45\textwidth]{\includegraphics[height=3in]{gait_training.jpg}}%
	\hfill
...
\subsection{Different approaches to intent estimation}
\begin{figure}
	\centering
...
\begin{figure}
	...
	\caption{The model-order reduction of a human (a); (b) an anchor model, the neuromuscular Force Modulated Compliant Hip \cite{davoodi2019template} where the hip is actuated using Hill-type muscle models, (c) a template model, Bipedal Spring-Loaded Inverted Pendulum model \cite{geyer2006compliant} }\label{fig:abstraction}
\end{figure}
