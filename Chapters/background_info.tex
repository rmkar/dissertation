...
\section{Models of legged locomotion}\label{sec:models}
...
\begin{equation}
	...
	\end{bmatrix}
\end{equation}
Were the feet assumed to have mass, this reset map would also handle the impact event. The second map is applied as follows when the LO guard \eqref{eq:glo} is satisfied
%
%\begin{gather}
...
%\end{equation}
%
where $ \mathbf{f}(\x_{c}^-,\p_{A}^-) $ computes the lateral position and velocity of the CoM, as the state of the CoM is measured relative to the new stance foot. Thus, the B-SLIP model can exhibit periodic gaits by alternately switching through SS and DS phases.
...
The state estimates and estimate covariance $ \P(t) $ are propagated in continuous time such that
\begin{eqnarray}
